...
def gaze_detection(VID_NAME=0,SHIFT=None,USER_FACE_WIDTH=150,BLINK_THRESHOLD=0.51,MIN_DETECTION_CONFIDENCE=0.9,MIN_TRACKING_CONFIDENCE=0.9): 
# Inputs:
# VID_NAME: webcam if 0, video file if string
# SHIFT: [X,Y,WIDTH,HEIGHT] shift of video on subject
...
# MIN_DETECTION_CONFIDENCE = threshold of how confidently model must detect face to consider results valid
# MIN_TRACKING_CONFIDENCE = threshold of how confidently must detect gaze to consider results valid
...
# Video Parameters
    IS_RECORDING = True  # Controls whether data is being logged
    SHOW_ON_SCREEN_DATA = True # If True, display blink count and head pose angles on the video feed.
...
# User-configurable parameters
    ...
    PRINT_DATA = False  # Enable or disable the printing of data to the console for debugging.
    ENABLE_HEAD_POSE = True # Enable the head position and orientation estimator.
...
#cap = cv.VideoCapture(cam_source)
    if isinstance(VID_NAME, int):
        ...
        cap_og = cv.VideoCapture(0)
        cap_met = cv.VideoCapture(0)
    elif isinstance(VID_NAME, str):
        cap = cv.VideoCapture(VID_NAME)
        cap_og = cv.VideoCapture(VID_NAME)
        cap_met = cv.VideoCapture(VID_NAME)

    # Get the properties of the videos
    frame_width = int(cap_og.get(cv.CAP_PROP_FRAME_WIDTH))
    frame_height = int(cap_og.get(cv.CAP_PROP_FRAME_HEIGHT))
    fps = int(cap_og.get(cv.CAP_PROP_FPS))
    num_frames = int(cap_og.get(cv.CAP_PROP_FRAME_COUNT))

    # Create a video writer
    output_video = cv.VideoWriter('output_video.mp4', cv.VideoWriter_fourcc(*'mp4v'), fps, (frame_width * 2, frame_height * 2), isColor=True)
...
# Main loop for video capture and processing
    try:
        ...
        while True:
            ...
            ret_met, frame_met = cap_met.read()
            ...
            # Writing the on screen data on the frame
                if SHOW_ON_SCREEN_DATA:
                    if IS_RECORDING:
                        cv.circle(frame_met, (30, 30), 10, (0, 0, 255), -1)  # Red circle at the top-left corner
                    cv.putText(frame_met, f"Blinks: {TOTAL_BLINKS}", (30, 80), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                    if ENABLE_HEAD_POSE:
                        cv.putText(frame_met, f"Pitch: {int(pitch)}", (30, 110), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                        cv.putText(frame_met, f"Yaw: {int(yaw)}", (30, 140), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)
                        cv.putText(frame_met, f"Roll: {int(roll)}", (30, 170), cv.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2, cv.LINE_AA)


            # Resize frames if necessary
            frame1 = cv.resize(frame_og, (frame_width, frame_height))
            frame2 = frame  # No need to resize the cropped frame
            frame3 = cv.resize(frame_met, (frame_width, frame_height))
            frame4 = cv.resize(frame, (frame_width, frame_height))

            # Determine the aspect ratio of the cropped frame
            aspect_ratio = frame.shape[1] / frame.shape[0]

            # Calculate the width of the resized cropped frame to maintain its aspect ratio
            new_width = int(frame_height * aspect_ratio)

            # Resize the cropped frame to match the new width
            frame2 = cv.resize(frame, (new_width, frame_height))

            # Pad the resized cropped frame to match the width of the original frames
            padding = (frame_width - new_width) // 2
            frame2 = cv.copyMakeBorder(frame2, 0, 0, padding, padding, cv.BORDER_CONSTANT, value=(0, 0, 0))

            # Create a 2x2 grid
            top_row = cv.hconcat([frame1, frame2])
            bottom_row = cv.hconcat([frame3, frame4])
            combined_frame = cv.vconcat([top_row, bottom_row])
